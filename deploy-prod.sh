...
# Set configmap for env
kubectl create configmap lotdiz-config --from-env-file=.env -n prod
...
if kubectl rollout status deployment/apigateway-deployment -n prod | grep -q "successfully rolled out"; then
  echo "Success to create new kubernetes deployment resources"

  kubectl annotate deployment/apigateway-deployment kubernetes.io/change-cause="Change image to ${IMAGE_TAG}"
  ...
  kubectl rollout undo deployment/apigateway-deployment -n prod
  if kubectl rollout status deployment/apigateway-deployment -n prod | grep -q "successfully rolled out"; then
    echo "Rollback was successful"

    kubectl annotate deployment/apigateway-deployment kubernetes.io/change-cause="Rollback to previous image"
  else
